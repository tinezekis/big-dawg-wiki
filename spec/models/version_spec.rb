...
describe Version do
  ...
  let(:version) { Version.new(article: :article, updating_author: :admin) }
  ...
  describe 'when initialized' do
    ...
    it 'has an original author (through the article)' do
      expect(version.orig_author).to eq(:user)
    end
  ...
  describe 'new versions of the same article' do
    ...
    it 'gets marked as the most recent version' do
      expect(update.is_most_recent).to be true
    end

    it 'marks all other versions as NOT the most recent' do
